  ...
  outputs =
    ...
    let
      ...
      normalizeRustVersion =
        ...
        in
        if majorMinorPatch != null then
          version
        else if majorMinor != null then
          version + ".0"
        else
          throw "Invalid Rust version format: ${version}. Expected format: X.Y.Z or X.Y";
    in
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        ki-editor-wayland = craneLib.buildPackage (
          waylandCommonArgs
          // {
            cargoArtifacts = craneLib.buildDepsOnly waylandCommonArgs;
          }
        );
      in
      {
        packages.default = ki-editor;
        packages.ki-editor-wayland = ki-editor-wayland;
