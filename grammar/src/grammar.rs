...
pub fn fetch_grammars(mut grammars: Vec<GrammarConfiguration>) -> Result<()> {
...
fn fetch_grammar(grammar: GrammarConfiguration) -> Result<FetchStatus> {
    ...
    } = grammar.source
    {
        ...
        if get_remote_url(&grammar_dir) != Some(remote.clone()) {
            set_remote(&grammar_dir, &remote)?;
        }

        // ensure the revision matches the configured revision
        if get_revision(&grammar_dir) != Some(revision.clone()) {
