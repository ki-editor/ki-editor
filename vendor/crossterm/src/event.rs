...
bitflags! {
    ...
    pub struct KeyboardEnhancementFlags: u8 {
        /// Report committed text separately from key identity.
...
impl Event {
    ...
    pub fn as_key_event(&self) -> Option<KeyEvent> {
        match self {
            Event::Key(event) => Some(event.clone()),
            _ => None,
        }
    }
    ...
    pub fn as_key_press_event(&self) -> Option<KeyEvent> {
        match self {
            Event::Key(event) if self.is_key_press() => Some(event.clone()),
            _ => None,
        }
    }
    ...
    pub fn as_key_release_event(&self) -> Option<KeyEvent> {
        match self {
            Event::Key(event) if self.is_key_release() => Some(event.clone()),
            _ => None,
        }
    }
    ...
    pub fn as_key_repeat_event(&self) -> Option<KeyEvent> {
        match self {
            Event::Key(event) if self.is_key_repeat() => Some(event.clone()),
            _ => None,
        }
    }
    ...
    pub fn as_mouse_event(&self) -> Option<MouseEvent> {
        match self {
            Event::Mouse(event) => Some(event.clone()),
            _ => None,
        }
    }
...
pub struct KeyEvent {
    ...
    /// Text committed by the terminal, separate from the key identity.
