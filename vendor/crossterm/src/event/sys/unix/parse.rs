...
pub(crate) fn parse_csi_u_encoded_key_code(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    // it replaces the key code while retaining the physical modifier.
    if modifiers.contains(KeyModifiers::SHIFT) {
        if let Some(shifted_c) = shifted_key {
            keycode = KeyCode::Char(shifted_c);
            // Preserve physical Shift for positional shortcuts.
        }
    }
...
mod tests {
    ...
    #[test]
    fn ghostty_ergol_events() {
        let star = key_event(b"\x1b[57454::111u");
        assert_eq!(
            star.code,
            KeyCode::Modifier(ModifierKeyCode::IsoLevel5Shift)
        );
        assert_eq!(star.base_layout_code, Some(KeyCode::Char('o')));
        assert_eq!(star.kind, KeyEventKind::Press);
        assert_eq!(star.text, None);
        let release = key_event(b"\x1b[57454::111;1:3u");
        assert_eq!(release.kind, KeyEventKind::Release);
        assert_eq!(release.base_layout_code, star.base_layout_code);
        let composed = key_event(b"\x1b[97;;224u");
        assert_eq!(composed.code, KeyCode::Char('a'));
        assert_eq!(composed.text.as_deref(), Some("à"));
        let multiple = key_event(b"\x1b[97;1:2;97:776u");
        assert_eq!(multiple.kind, KeyEventKind::Repeat);
        assert_eq!(multiple.text.as_deref(), Some("a\u{308}"));
        let shifted = key_event(b"\x1b[99:67:119;2;67u");
        assert_eq!(shifted.code, KeyCode::Char('C'));
        assert!(shifted.modifiers.contains(KeyModifiers::SHIFT));
        assert_eq!(shifted.base_layout_code, Some(KeyCode::Char('w')));
        assert_eq!(shifted.text.as_deref(), Some("C"));
        assert!(parse_event(b"\x1b[97;;1114112u", false).is_err());
        assert!(parse_event(b"\x1b[97;;55296u", false).is_err());
    }
