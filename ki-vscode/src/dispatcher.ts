...
export type EventParams<T extends EventName> = T extends
    | "document.open"
    | "document.save"
    | "document.close"
...
export class Dispatcher implements vscode.Disposable {
    ...
    private eventHandlers: Map<
        EventName,
        ((params: EventParams<EventName>) => void)[]
    > = new Map();
    ...
    private processKiNotification(
        ...
        if (tag === "mode.change") {
            this.logger.log(
                `Processing mode change: ${JSON.stringify(params)}`,
            );
        }
        ...
        // Special handling for selection updates
            ...
            this.logger.log(
                `Processing selection update from Ki: ${JSON.stringify(params)}`,
            );
        }
