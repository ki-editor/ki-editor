...
For non-repeating actions in multicursor mode while in the momentary layer, user may tap the space key, which opens the Multi-cursor menu:

<KeymapFallback filename="Multi-cursor Menu"/>

## 1. Movements (`Add Curs`)
