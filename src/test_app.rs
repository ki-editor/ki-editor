...
pub(crate) enum ExpectKind {
    ...
    CurrentKeyboardLayout(KeyboardLayoutKind),
}
...
impl ExpectKind {
    ...
    fn get_result(&self, app: &mut App<MockFrontend>) -> anyhow::Result<(bool, String)> {
        ...
        Ok(match self {
            ...
            CurrentKeyboardLayout(expected) => {
                contextualize(expected, app.context().keyboard_layout_kind())
            },
        })
    }
}
...
#[test]
fn keyboard_layout_picker() -> anyhow::Result<()> {
    execute_test(|s| {
        Box::new([
            App(OpenFile {
                path: s.main_rs(),
            ...
            Expect(ExpectKind::CurrentKeyboardLayout(
                KeyboardLayoutKind::Qwerty,
            )),
            App(HandleKeyEvents(
                keys!("space t w o r k m a n enter").to_vec(),
            )),
            Expect(ExpectKind::CurrentKeyboardLayout(
